...
const muiClassNameRegexp = /([a-zA-Z0-9]*)-([a-zA-Z0-9]*)-([0-9]*)/;
...
const replaceJssClassNames = (elements) => {
  elements.forEach((element) => {
    ...
    // Material UI generates class names that follow this format:
    // {componentName}-{ruleName}-{ruleCounter}
    // the non-deterministic part of it is "-{ruleCounter}"
    ...
    const deterministicClassNames = classNameProp
      ...
      .map((className) => {
        ...
        if (!muiClassNameRegexp.test(className)) return className;

        const lastDashPosition = className.lastIndexOf('-');

        return className.substring(0, lastDashPosition);
      });
